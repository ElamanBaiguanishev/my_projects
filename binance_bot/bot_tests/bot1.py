...
from db1 import add_data, replace_data, clean_data, get_data, init_db
from tools import check_validate

bot = Bot(token='6324885383:AAG6t1M2ii1qLwBjEaxY7QMwW7MMt_sHIKw')
storage = MemoryStorage()
dp1 = Dispatcher(bot, storage=storage)
...
@dp1.message_handler(Command('start'), state=None)
...
@dp1.message_handler(Command('processing'), state=None)
...
@dp1.message_handler(Command('stop'), state=State.STARTED)
...
@dp1.message_handler(Command("db"), state=None)
...
@dp1.message_handler(state=State.STARTED_DB)
...
@dp1.message_handler(state=[State.BASE, State.ADD], content_types=types.ContentType.DOCUMENT)
...
@dp1.message_handler(state=State.CLEAN)
...
@dp1.message_handler(state=[State.BASE, State.ADD])
