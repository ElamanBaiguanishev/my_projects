...
from db2 import add_data, replace_data, clean_data, get_data, init_db
from tools import check_validate

bot = Bot(token='6318444504:AAEaOhDTL9smyxw_u5FAbE3ExhmjzMZfqAM')
storage = MemoryStorage()
dp2 = Dispatcher(bot, storage=storage)
...
@dp2.message_handler(Command('start'), state=None)
...
@dp2.message_handler(Command('processing'), state=None)
...
@dp2.message_handler(Command('stop'), state=State.STARTED)
...
@dp2.message_handler(Command("db"), state=None)
...
@dp2.message_handler(state=State.STARTED_DB)
...
@dp2.message_handler(state=[State.BASE, State.ADD], content_types=types.ContentType.DOCUMENT)
...
@dp2.message_handler(state=State.CLEAN)
...
@dp2.message_handler(state=[State.BASE, State.ADD])
