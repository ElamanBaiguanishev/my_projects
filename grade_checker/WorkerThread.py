            ...
            while True:
                try:
                    ...
                    self.driver.execute_script('document.body.innerHTML = arguments[0]',
                                               info_div.get_attribute('outerHTML') + div_element.get_attribute(
                                                   'outerHTML'))
                    ...
                    if self.ip:
                        print(valid, "Успешно, перезагружаю")
                        change_ip()
                    print(valid, "конец")
                    break
                except:
                    if count == 2:
                        print('Слишком много попыток, прерываю')
                        sc = lambda X: self.driver.execute_script('return document.body.parentNode.scroll' + X)
                        ...
                        break
                    print("Меняю айпи адресс из-за ошибки")
                    change_ip()
