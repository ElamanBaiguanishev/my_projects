...
def init_proxy(id_: int):
    try:
        ...
        cursor.execute('INSERT INTO proxy (id, text)  VALUES (?, ?)', (id_, "[]"))
...
def proxy(id_: int, data_):
    try:
        conn = sqlite3.connect('data.db')
        cursor = conn.cursor()

        cursor.execute('INSERT INTO proxy (id, text)  VALUES (?, ?)', (id_, json.dumps(data_)))
...
def replace_proxy(id_: int, data_):
    ...
    try:
        ...
        sql_query = f"UPDATE proxy SET text = ? WHERE id = ?"
...
def get_proxy(id_: int):
    try:
        conn = sqlite3.connect('data.db')
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM proxy WHERE id = ?', (id_,))
