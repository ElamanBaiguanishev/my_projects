...
second_name = input("Введите первый параметр: ")
first_name = input("Введите второй параметр: ")
adapter = input("Введите сетевой адаптер: ")
...
def main(second_param, first_param, adapter):
    ...
    print(f" Содержимое папки C:\\Users\\{first_param}\\{name_folder}")
    print("")
    date_folder = f"{time.strftime(f'%d.%m.%Y %H:{get_min(M - 1)}')}"
    c = c + random.randint(-50, 100)
    d = d + random.randint(-50, 100)
